...
public class BitOp extends AbstractCodedOp {
	...
	public BitExp getUnconditionalEffects() {
		final BitExp uc_effect = new BitExp();
		for (CondBitExp c_effect : this.effects) {
			if (c_effect.getCondition().isEmpty()) {
				final BitExp cond_eff = c_effect.getEffects();
				uc_effect.getPositive().or(cond_eff.getPositive());
				uc_effect.getNegative().or(cond_eff.getNegative());
			}
		}
		return uc_effect;
	}
